...
object NodeRef {
  ...
  def apply[A](a: A, ref: Node[A], next: Node[A], orig: Node[A] = null)(implicit
      ord: A => Ordered[A]
  ): Node[A] =
  ...
  def constructEmptyEx[A](
      external_next_last: Node[A],
      ref_first: Node[A] = null
  )(implicit
      ord: A => Ordered[A]
  ): NodeRef[A] =
...
class Node[A](var a: A = null.asInstanceOf[A], next: Node[A])(implicit
    ord: A => Ordered[A]
...
private class FirstRef[A](
  ...
  override def compare(that: Node[A]): Int =
    -1 // always smaller of the smallest
...
/// generic sentinel nodes, First: smallest value node, and Last: greatest value node
private class First[A](next: Node[A])(implicit ord: A => Ordered[A])
    extends Node[A](null.asInstanceOf[A], next) {
  override def compare(that: Node[A]): Int =
    -1 // always smaller of the smallest
...
class Last[A](implicit ord: A => Ordered[A])
    extends Node[A](null.asInstanceOf[A], null) {
  override def compare(that: Node[A]): Int = 1 // bigest ever
...
object OrderedList {
  ...
  def findClosestLesserValue2[A](
    ...
    if (
      curr_marked(0) == true && (pred.compareAndSet(
        curr,
        succ,
        false,
        false
      ...
      if (marked(0) == false) {
        findClosestLesserValue2(
          repeat,
          repeat,
          pred2,
          a
        ) // repeat the whole thing from start node
      } else {
  ...
  def insertInRange[A](
    ...
    if (pred.eq(to) == false) {
      ...
      // process lazy delete
      if (
        marked(0) == true && (pred.compareAndSet(
          curr,
          succ,
          false,
          false
        ...
        if (
          marked(0) == true
        ) // special case, first element no more, cannot run usual repeat
          {
            // abort, rewind to "to"
            insertInRange(
              start,
      ...
      ) // check if we already inserted something but still counting the range
        {
          count(0) = count(0) + 1
          insertInRange(
            start,
        ...
        curr.lt(new_n.a)
      ) // counting, but we did not find a place to insert yet
        {
          count(0) = count(0) + 1
          insertInRange(
            start,
        ...
        if ((curr.isLast) || (new_n.compareTo(curr) != 0)) {
          ...
          if (pred.compareAndSet(curr, new_n, false, false) == false) {
            ...
            insertInRange(
              ...
              added,
              true
            ) // mark as done but keep counting
          }
        } else
          insertInRange(
            start,
          ...
          ) // ignore dupe, counting with done = true
      }
    } else if (done == true) {
      ...
      done // will be false - for drop cases, all other cases: it cannot reach the end and be false
  }
  ...
  def removeFromRange[A](
      ...
      removeFrom: Array[
        Node[A]
      ], // can be null if removed after initial/start node
        ...
        if (curr.compareTo(Node_a) == 0) {
          ...
          if (remove(0) == true) {
            ...
            if (start.eq(pred) == false)
              removeFrom(0) =
                pred // save the node before removed one. we may need it for substitution in case if node was removed from reference layers
            else
              removeFrom(0) =
                null // nothing to give, pred is very first already, we cannot be out of bound
          ...
          if (pred.lt(Node_a.a) == true && curr.lt(Node_a.a) == false) {
            removeFrom(0) =
              pred // pred less then "a" but curr already more then "a", so save the border line pred
          }
          removeFromRange_t(
            start,
            curr,
