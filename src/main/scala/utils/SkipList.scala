...
object ValuePair {
  def apply[A <: Comparable[A], B <: Comparable[B]](key: A, value: B = null)(
...
class ValuePair[A <: Comparable[A], B](val key: A, val value: B)(implicit
    ord: A => Ordered[A]
) extends Ordered[ValuePair[A, B]] {

  override def compare(that: ValuePair[A, B]): Int =
    key.compareTo(that.key)
}

class ReversedValuePair[A <: Comparable[A], B](val key: A, val value: B)(
    implicit ord: A => Ordered[A]
) extends Ordered[ReversedValuePair[A, B]] {
  override def compare(that: ReversedValuePair[A, B]): Int =
    key.compareTo(that.key) * -1
}

class StringIgnoreCaseValuePair[B](val key: String, val value: B)
    extends Ordered[StringIgnoreCaseValuePair[B]] {
  ...
  override def compare(that: StringIgnoreCaseValuePair[B]): Int = {
    ...
    nocaseKey.compareTo(nocase_thatKey)
  }
}
...
class ReversedStringIgnoreCaseValuePair[B](val key: String, val value: B)
  ...
  override def compare(that: ReversedStringIgnoreCaseValuePair[B]): Int = {
    ...
    val nocaseKey = key.toLowerCase();
    val nocase_thatKey = that.key.toLowerCase();
    nocaseKey.compareTo(nocase_thatKey) * -1
  }
}

class MultiValuePair[A <: Comparable[A], B](val key: A, val value: B)(implicit
    ord: A => Ordered[A]
  ...
  override def compare(that: MultiValuePair[A, B]): Int = {
    val res = key.compareTo(that.key)

    if (res == 0) {
      if (value == that.value)
        0 // find or prevent duplicate or remove only if value matches requested value.
      else 1
    } else res
...
class MultiValuePairReversed[A <: Comparable[A], B](
    val key: A,
    val value: B
)(implicit
    ord: A => Ordered[A]
  ...
  override def compare(that: MultiValuePairReversed[A, B]): Int = {
    val res = key.compareTo(that.key) * -1

    if (res == 0) {
      if (value == that.value)
        // find or prevent duplicate or remove only if value matches requested value.
        0
      else 1
    } else res
...
class MultiValuePairOrd[A <: Comparable[A], B <: Comparable[B]](
    val key: A,
    val value: B,
    val reversed_values: Boolean = false
)(implicit
    ord: A => Ordered[A],
    ord2: B => Ordered[B]
  ...
  override def compare(that: MultiValuePairOrd[A, B]): Int = {
    ...
    if (res == 0) {
      value.compareTo(that.value) * mult
    } else res
...
/// Multvalue pair sorted by key and value - its Map key and value key
class MultiValuePairReversedOrd[A <: Comparable[A], B <: Comparable[B]](
    val key: A,
    val value: B,
    val reversed_values: Boolean = false
)(implicit
...
class TwoKeysMultiValuePairOrd[A1 <: Comparable[A1], A2 <: Comparable[
  A2
], B <: Comparable[B]](
    val key1: A1,
    val key2: A2,
  ...
  override def compare(that: TwoKeysMultiValuePairOrd[A1, A2, B]): Int = {
    ...
    val res = key1.compareTo(that.key1)
    if (res == 0) {
      val res1 = key2.compareTo(that.key2)
      if (res1 == 0) {
...
class SkipList[A <: Comparable[A]](implicit ord: A => Ordered[A]) {
  ...
  def debug_test_ref(list: Node[A], ref: Node[A]): Boolean = {
    ...
    if (result == false) {
      ...
      println(
        "NOT THERE=" + ref.a + " orig= " + ref.getOrig.a + "with marked = " + marked(
          0
        )
      )
    }
      ...
      if (status == false) {
        println(
          Thread
            .currentThread()
            .getId()
            .toString + " for Layer:" + curLayer + " FAILED"
        )
        return false
      }
      println(
        Thread
          .currentThread()
          .getId()
          .toString + " for Layer:" + curLayer + " OK"
      )
  ...
  def print(out: StringBuilder) = {
    ...
    while (cur_layer.hasRef) {
      ...
      var maxRangeCountShow =
        if (maxRangeCount > FACTOR * 10) "*" else maxRangeCount.toString
      out.append(
    ...
    out.append(
      "Bottom: " + OrderedList.countRange[A](cur_layer, _lastRef) + "\n"
    )
  }
  ...
  def foreachFrom(
    ...
    while (!curr.isLast && drop == false) {
      ...
      curr = curr.get(
        marked
      ) // we don't check first element if marked, the rest are just duplicated extra top level checks in an attempt to provide the most recent version
    }
  }
  ...
  private def _findClosestLT(
    ...
    if (closestTop == null) {
      ...
      val Node_a = Node(
        a,
        null
      ) // for gen cases we cannot use "==", just reuse Ordered by applying Node as wrapper
  ...
  final def add(a: A): Boolean = {
    ...
    if (newTopRef(0) != null) {
      ...
      if (isEmpty(old_top) == false) {
        ...
        val new_top = NodeRef.constructEmptyEx[A](
          external_next_last = _lastRef,
          ref_first = old_top
        )
        ...
        top.compareAndSet(
          old_top,
          new_top
        ) // no issues if we could not do that, some other parallel thread did
      }
  ...
  final def remove(a: A): Boolean = {
    ...
    val merge = Array[Boolean](
      false
    ) // merge means that we need to remove one item from upper layer
    val colapse = Array[Boolean](
      false
    ) // colapse means that one of the upper layers don't have members anymore and we cut it off and all the preceding higher layers
    val removed = Array[Boolean](
      false
    ) // flag when somthing was removed from the list, it's possible to call function many times for not present items, so it can be false
  ...
  private def _add(
    ...
    if (from.hasRef == false) {
      ...
      if (added(0) == false) {
        status = OrderedList.insertInRange(
          from,
          from,
          to,
          Node(a, null),
      ...
      if (
        lowerNodeFrom
          .isMarked() || (!lowerNodeFrom.isFirst && lowerNodeFrom.hasRef && lowerNodeFrom_o.isMarked)
      ) {
      ...
      if (
        lowerNodeTo.isMarked() || (!lowerNodeTo.isLast && lowerNodeTo.hasRef && lowerNodeTo_o
          .isMarked())
      ) {
        if (
          closestTopGetReference.isFirst == false && closestTopGetReference.isLast == false
        )
          closestTopGetReference.set(
            closestTopGetReference.getReference(),
            true
          )
        return false
      }
  ...
  private def _remove(
    ...
    if (from.hasRef == false) {
      ...
      val closestTop = OrderedList.findClosestLesserValue2(
        from,
        from,
        predValueToClosestLesser,
        al.a
      )
      ...
      if (status == true) {
        val prev_removedFrom = removeFrom(
          0
        ) // if item wasn't removed ( not found ), removeFrom will have closest node anyway, so we can recover.
